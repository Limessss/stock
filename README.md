...
“情绪周期”页面默认只使用本地 Parquet 行情；打开页面或刷新页面不会访问外部接口。点击 **同步至最新** 后，系统会以本地行情缓存的最新交易日为终点：首次使用批量同步最近 30 个交易日，已有数据时只补齐该窗口内缺少的日期，并保存本地涨跌家数、涨跌停、百日新高和连板梯队。单日日期选择器仅用于查看历史快照。

“区间涨幅”默认展示截至个股行情最新交易日的近 10 个交易日排行；列表仅包含起止日都有有效前复权收盘价的股票。
...
凭据只保存在本机 `.env`，不要提交到仓库。外部原始响应按“接口 + 日期 + 参数”缓存；只有主动点击同步才可能发起请求，普通刷新不会请求。“重建近 30 日”仅用于缓存异常或接口映射调整后重新抓取。
...
| GET | `/api/sentiment/interval-gains?days=10` | 近 10 个交易日的有效股票涨幅排行 |
| GET | `/api/sentiment/{date}` | 单日情绪周期快照（只读缓存） |
| POST | `/api/sentiment/sync-latest` | 补齐本地行情最新 30 个交易日的缺失快照，支持 `force=true` |
